...
*
* -------------------------------------
* A2D::QuickList push_back - Integer[10000000 iterations]
* Time taken : 0.736000000s
* -------------------------------------
* A2D::QuickList remove - Integer[1 iterations]
* Time taken : 0.426000000s
* -------------------------------------
* A2D::QuickList push_back - Integer[10000000 iterations]
* Time taken : 0.372000000s
* -------------------------------------
* A2D::QuickList max_remove_all - Integer[1 iterations][Competing against std::vector!!!!]
...
#define _LIST_MAX_BYTE_ALLOCATION					 4000 // Heap allocation	  
#define _LOG(f, ...)								 //SYSOUT_F(f , __VA_ARGS__ )		  
...
namespace A2D{
	...
	public:
		...
		public:
			...
			bool has_next()
			{
				return m_node->right != NULL && m_size > -1;
			}
		};
	...
	private:
		...
		void remove_node(Node<T> * node)
		{
			...
			if (m_fragments_available_index > 0)
			{
				...
				found = true;
				fragment = m_fragments[--m_fragments_available_index];  // decrement
			}
			...
			fragment = found ? fragment : m_fragments[m_fragments_cache_total++] = node;
			fragment = node;
